...
public class MainFrame implements ActionListener
{
	...
	private String infoMess = "Press any key to start game";
	...
	private void initiateComponents()
	{
		...
		frame = new JFrame("BattleShipProject");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(play1.takeAim(play2, x, y))
			text.setText(play1.getName() + " Found The Enemy Spot!\n");
		else
			text.setText(play1.getName() + " missed the target\n");
		int updateCount2 = count2.destroyedUpdate();
		if(updateCount2 != -1)
			text.append(play2.getName() + "'s " + updateCount2 + " ship was destroyed!\n");
		changeButtonE(x, y);
		
		if(play2.takeAim(play1, 0, 0))
			text.append(play2.getName() + " Found your spot\n");
		else
			text.append(play2.getName() + " missed the target\n");
		int updateCount1 = count1.destroyedUpdate();
		if(updateCount1 != -1)
			text.append(play1.getName() + "'s " + updateCount1 + " ship was destroyed!\n");
		changeButtonM();
	...
	private void changeButtonE(int x, int y)
	{
		...
		if(txt.equals("X"))
			buttonE[x][y].setBackground(new Color(0,255,127));
		else
			buttonE[x][y].setBackground(new Color(255,192,203));
	}
